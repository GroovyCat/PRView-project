...
def get_tags_all(text, noun_count):
    ...
    fig.savefig('C:/Python_basic/env/prv_project/prv_app/static/img_url_all/url_all.png') # 해당 이름으로 png 저장

def get_tags_pos(text, noun_count):
    spliter = Okt()
    nouns = spliter.nouns(text) # nouns 함수를 통해서 text에서 명사만 분리/추출
    count = Counter(nouns) # Counter 객체를 생성하고 참조변수 nouns할당
    return_list = {} # 명사와 빈도를 저장하기 위한 딕셔너리
...
def get_tags_neg(text, noun_count):
    spliter = Okt()
    nouns = spliter.nouns(text) # nouns 함수를 통해서 text에서 명사만 분리/추출
    count = Counter(nouns) # Counter 객체를 생성하고 참조변수 nouns할당
    return_list = {} # 명사와 빈도를 저장하기 위한 딕셔너리
